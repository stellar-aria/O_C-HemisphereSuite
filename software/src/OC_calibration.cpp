...
void calibration_draw(const CalibrationState &state) {
  ...
  weegfx::coord_t y = OC::menu::CalcLineY(0);

  static constexpr weegfx::coord_t kValueX = OC::menu::kDisplayWidth - 30;

  graphics.setPrintPos(OC::menu::kIndentDx, y + 2);
  switch (step->calibration_type) {
    ...
    case CALIBRATE_SCREENSAVER:
      ...
      OC::menu::DrawEditIcon(kValueX, y, state.encoder_value, step->min, step->max);
      break;
  ...
  weegfx::coord_t x = OC::menu::kDisplayWidth - 22;
  y = 2;
  ...
  graphics.drawStr(0, OC::menu::kDisplayHeight - OC::menu::kFontHeight - 1, step->footer);

  static constexpr uint16_t step_width = (OC::menu::kDisplayWidth << 8 ) / (CALIBRATION_STEP_LAST - 1);
  graphics.drawRect(0, OC::menu::kDisplayHeight - 2, (state.step * step_width) >> 8, 2);
