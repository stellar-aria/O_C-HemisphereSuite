...
void ApplicationBase::BaseController() {
  for (size_t ch = 0; ch < 4; ch++) {
    ...
    inputs[ch] = oc::ADC::raw_pitch_value(ch);
...
void ApplicationBase::Out(int ch, int value, int octave) {
  oc::DAC::set_pitch((size_t)ch, value, octave);
...
bool ApplicationBase::Gate(int ch) {
  bool high = 0;
  if (ch == 0) high = oc::DigitalInputs::read_immediate(1);
  if (ch == 1) high = oc::DigitalInputs::read_immediate(2);
  if (ch == 2) high = oc::DigitalInputs::read_immediate(3);
  if (ch == 3) high = oc::DigitalInputs::read_immediate(4);
  return high;
}
...
bool ApplicationBase::Clock(int ch) {
  ...
  if (clock_m->IsRunning() && clock_m->GetMultiply(ch) != 0)
    ...
    if (ch == 0) clocked = oc::DigitalInputs::clocked(1);
    if (ch == 1) clocked = oc::DigitalInputs::clocked(2);
    if (ch == 2) clocked = oc::DigitalInputs::clocked(3);
    if (ch == 3) clocked = oc::DigitalInputs::clocked(4);
  }
